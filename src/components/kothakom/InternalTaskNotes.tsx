...
interface InternalTaskNotesProps {
  taskId: string;
  className?: string;
}

export function InternalTaskNotes({ taskId, className }: InternalTaskNotesProps) {
  ...
  const notesCollectionRef = useMemoFirebase(() => {
    ...
    return collection(firestore, 'internal_tasks', taskId, 'notes');
  }, [firestore, taskId]);
  ...
  const handleSaveEdit = () => {
    ...
    const noteRef = doc(firestore, 'internal_tasks', taskId, 'notes', editingNoteId);
  ...
  const handleDeleteNote = (noteId: string) => {
    if (!firestore) return;
    const noteRef = doc(firestore, 'internal_tasks', taskId, 'notes', noteId);
